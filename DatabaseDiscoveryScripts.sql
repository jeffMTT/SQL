/*
	...
	Assortment of diagnostic queries when working with an unfamiliar database. 
*/

/* 
	Exploring you Servers - Basic Server Info
*/
...
/*
	...
	Linked Servers are database connections set up to allow communication from SQL Server to other data servers. 
	Distributed queries can be ran against these linked servers. It's good to know if your database server is an isolated self-contained 
	database server or if there are links to other database servers
*/
EXEC sp_helpserver; 
--OR  
...
/*
	...
	First step is to take inventory of all the databases found on the server. 
	Note the four or five system databases (master,  model,  msdb,  tempdb, and   distribution if you are using replication). 
	You may want to exclude these system databases in future queries. It is very easy to see a list of database directly from 
	SQL Server Management Studio SQL Server (SSMS). However, these simple database queries are building blocks for more complicated queries. 
*/
EXEC sp_helpdb; 
--OR  
EXEC sp_Databases; 
...
/*
	...
	It's a good idea to have an understanding of what databases are being used at any point in time, 
	especially if you are experiencing performance problems. 
*/
...
/*
	...
	Let's now drill down and see how we can gather information about the objects in each of our databases, using various catalog views and 
	Dynamic management Views. Most of the queries presented in this section let's look at the individual database so remember to change the current 
	database in SSMS or by a Use database; command. Remember to you can always check the current default database with Select DB_NAME();.
	The  sys.objects system table is one of the key system tables for gathering a lot of information on the objects that comprise your data model, 
	with  sys.objects.type being the key column on which to filter.
	
	Other catalog views, such as  sys.tables and   sys.views, inherit from sys.objects and provide the information for that particular object type. 
	With these views, plus the OBJECTPROPERTY metadata function, we can uncover a great deal of information on each of the objects that make up our 
	database schemas.
*/
...
/*
	...
	Physical location and drive of the current database file, including the master database file (*.mdf) and the Log database file (*.ldf) 
	can be found using these queries.
*/
EXEC sp_Helpfile; 
--OR  
SELECT  @@Servername AS Server ,
...
/*
	...
	Of course, the Object Explorer in SSMS provides a convenient list of the tables in a specific database, but using scripts we can unveil 
	information that isn't easily available via the GUI. The ANSI Standard approach is to use the  INFORMATION_SCHEMA views, but they won't 
	return information regarding objects that are not part of the Standard (such as triggers, extended properties, and so on), so use of the 
	SQL Server Catalog views is common.
*/
...
--Row counts by table - dynamically generated tables
...
/*
	...
	sp_msforeachtable is an undocumented Microsoft function that will loop through all the tables in a database executing a query, and replacing '?' 
	with each table name. There is also a similar database level function called  sp_msforeachdb. 
	
	There are some known issues with these undocumented functions, as they do not handle special characters in the object names. 
	For example, table or database names containing a dash character ("-") will cause the stored procedure to fail.
...
/*
	A faster way to get row counts - use the clustered index. May not be exact.
*/
...
/*
	...
	Working with heap tables is like working with a flat file, instead of a database. If you want to guarantee a full table scan for any 
	and all queries, use a heap table. My general recommendation would be to add a primary key clustered index to all heap tables.
*/
...
/*
	...
	Knowing which tables have the most reads and writes is another important piece of information when performance tuning your database. 
	Previously, we examined queries to return the row counts for each table. The following examples show the number of table reads and writes.

	Note these statistics from Dynamic Management Views are cleared out each time SQL Server restarts (wait and latch statistics can also be 
	cleared out manually). The longer the server has been up, the more reliable the statistics. I have a lot more confidence with statistics that 
	are over 30 days (assumes the tables have been through a month end cycle) and a lot less confidence if they are less than 7 days. 
*/
...
/*
	...
	Views are scripted queries that are stored in the database. You can think of them as virtual tables. Data is not stored in the view but we 
	reference the view in our queries in exactly the same way we would reference a table.

	In SQL Server, we can even, in some circumstances, update data through a view. To make a view read only, one trick is to use  
	SELECT DISTINCT in the view definition. A view is only updateable if each row in the view maps unambiguously to a single row in the 
	underlying table. Any view that fails this criteria, such as any view built on more than one table, or that uses grouping, aggregations, 
	and calculations in its definition, will be read only.
*/
SELECT  @@Servername AS ServerName ,
        DB_NAME() AS DBName ,
        o.name AS ViewName ,
...
/*
	...
	A synonym is an "also known as (aka)" name for an object in the database. A few times in my career I have been asked to review a query 
	only to be scratching my head trying to figure out the table to which the query is referring. For example, consider a simple query  
	Select * from Client. I search for the table named  Client but I can't find it. OK, it must be a view then, search for view named  Client 
	and I still can't find it. I must have the wrong database? Turns out  Client is a synonym for a customer and the actual table is  Customer. 
	The marketing group wanted to refer to this table as Client so created a synonym. Thankfully, use of synonyms is rare, but they can cause confusion 
...
/*
	...
	Up to this point, we've examined scripts that gives us a 'high level' view of the objects that comprise our databases. Often though, 
	we'll want more in-depth knowledge of each table, including the columns, their data types and any default values defined, which keys, 
	constraints and indexes exist (or are missing) and so on. 
...
/*
	...
	It depends…I'm sure you have heard that before. I will review three different methods to 'reverse engineer' database dependencies. 
	The first method use the stored procedure  sp_msdependencies. The second method uses the foreign key systems tables. The third method uses a CTE. 
	...
	I have used this method several times when asked to perform archiving or deleting of data on some large database models. If you know the table dependencies, 
	you have a road map of the order in which you need to archive or delete records. Start with the table with the largest sequence number first, 
	then work backward from the largest number to the smallest number. Tables with the same sequence number can be removed at the same time. 
	This method does not violate any of the foreign key constraints, and thus allows you to move/delete records without temporarily dropping and rebuilding 
	constraints. 
*/
...
FROM    sys.foreign_key_columns
ORDER BY ParentTable, DependentTable
